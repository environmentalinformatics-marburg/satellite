...
test_that("calcSurfRef works as expected", {
  ...
  Lp <- calcPathRadDOS(DNmin = min(raster::getValues(l8[[2]])), 
                        bnbr = 2, band_wls = lut$l8_band_wl, coefs = coefs8,
                        ESun = ESun, scat_coef = -4)
  
  bands <- l8
  for(i in seq(raster::nlayers(bands))){
    bands[[i]] <- calibLandsat(bands[[i]], bnbr = i, coefs = coefs8)
  }
  
  sr <- calcSurfRef(bands_rad = bands, coefs = coefs8, ESun = ESun, Lp = Lp)
