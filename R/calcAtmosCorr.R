...
#' @param szen sun zenith angle in degrees.
...
#' #reading meta data to data frame
#' dfmeta <- compMetaLandsat(paste0(path,"LC81950252013188LGN00_MTL.txt"))
#' dfmeta <- dfmeta[!is.na(dfmeta$SPECTRUM) & dfmeta$SRES == 30, ]
#' #specify bandnr
#' n <- 2
#' #calculate solar irradiance
#' inesun <- calcTOAIrradRadRef(rad_max = dfmeta$RADMAX, ref_max = dfmeta$REFMAX, normalize = FALSE, esd = calcEartSunDist(dfmeta$DATE))
#' #calculate DOS path radiance
#' inpath_rad <- calcPathRadDOS(DNmin = min(getValues(landsat_8[[n]])), bnbr = dfmeta$LNBR[dfmeta$BID==n], band_wls = data.frame(LMIN = dfmeta$LMIN,
#' LMAX = dfmeta$LMAX), radm = dfmeta$RADM, rada = dfmeta$RADA, szen = dfmeta$SZEN, inesun, model = "DOS2", scat_coef = -4, dos_adjust = 0.01)
#' #calculate atmospheric correction for sat images
#' for(img in rep(1:length(names(landsat_8)))){
#'  tmp <- calcAtmosCorr(sensor_rad = landsat_8[[img]], inpath_rad[img], inesun[img], dfmeta$SZEN[img])
#'  landsat_8_ac <- stack(atcor,tmp)
#' }
